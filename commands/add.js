...
module.exports = {
    name: "add",
    description: "Add coins to your pocket",
    execute: async (client, message, args) => {
        const user = message.mentions.users.first();
        const CoinsToAdd = user ? parseInt(args[1]) : parseInt(args[0]);
        const MemberId = user ? user.id : message.author.id;

        if (!message.member.hasPermission("ADMINISTRATOR")) return;

        if (CoinsToAdd <= config.max.add) {
            if (CoinsToAdd >= config.min.add) {
                if (!isNaN(CoinsToAdd)) {
                    if (CoinsToAdd != null) {
